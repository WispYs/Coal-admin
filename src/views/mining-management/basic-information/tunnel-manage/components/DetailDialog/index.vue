...
import {
  getTunnelDetail,
  deleteTunnelDetailById,
  batchDeleteTunnelDetail
...
export default {
  ...
  methods: {
    __fetchData() {
      ...
      getTunnelDetail(query).then(response => {
    ...
    deleteClick(row) {
      this.$confirm('确定删除该数据?', '提示', {
        ...
        deleteTunnelDetailById(row.cjglTunnelDetailId).then(response => {
    ...
    deleteBatches() {
      const selectId = []
      this.multipleSelection.forEach(it => selectId.push(it.cjglTunnelDetailId))
      ...
      this.$confirm('确定删除所选中数据?', '提示', {
        ...
        batchDeleteTunnelDetail(selectId).then(response => {
          this.__fetchData()
          this.$message.success('删除成功')
        })
      })
