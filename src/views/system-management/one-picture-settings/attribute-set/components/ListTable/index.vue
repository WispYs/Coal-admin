  ...
  >
        ...
        <template slot-scope="scope">
          ...
          <template v-else>
            ...
            <template v-else-if="column.showType === 'underline'">
              <span class="underline-text" @click="textClick(scope.row[column.field])">{{ column.underlineText || '附件' }}</span>
            </template>
            <template v-else-if="column.dateFormat">
              <!-- 日期格式 -->
              <span>{{ scope.row[column.field] | parseTime(column.dateFormat) }}</span>
            </template>
            <template v-else>
              <span v-if="column.options">
                <!-- 针对动态获取的多级联动下拉菜单 -->
                <span v-if="column.subField">{{ scope.row[column.subField] }}</span>
                <span v-else>{{ filterField(column.options, scope.row[column.field]) }}</span>
              </span>
    ...
    <el-table-column v-if="config.actions && config.actions.length > 0" fixed="right" label="操作" :width="config.actionWidth || 160" align="center">
      <template slot-scope="scope">
        ...
        <el-button v-if="config.actions.indexOf('editReport') > -1 && scope.row.parentId != 0" type="text" size="small" @click="editReport(scope.row)">实体属性报表</el-button>
