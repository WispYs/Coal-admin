...
<script>
import { getFormwork, saveFormwork, getFormworkById, updateFormwork, delFormworkById } from '@/api/authority-management'
...
import { warnMouldManageconfig, NewsTemplateFilterConfig } from '@/data/authority-management'
  ...
  methods: {
    __fetchData() {
      ...
      const query = {
        ...
        keywordField: ['formworkCode', 'formworkName'],
        sort: {
          asc: ['sort']
        }
      }
      getFormwork(query).then(response => {
    ...
    initCreateConfig() {
      const createConfig = Object.assign({
        title: '新建',
        width: '1000px',
        form: this.warnMouldManageconfig.columns
      })
      return createConfig
    },
    // 初始化编辑窗口配置
    ...
    openDialog(name, row) {
      ...
      this[visible] = true
      if (row) {
        getFormworkById(row.gzyqFormworkId).then(response => {
    ...
    deleteClick(row) {
      this.$confirm('确定删除该模板?', '提示', {
        ...
        delFormworkById(row.gzyqFormworkId).then(response => {
    ...
    createSubmit(submitData) {
      ...
      saveFormwork(query).then(response => {
    ...
    editSubmit(submitData) {
      const query = Object.assign(submitData)
      updateFormwork(query).then(response => {
