<template>
  <div class="page-container">
    <filter-bar :config="certificatePhotoFilterConfig" @search-click="queryData" @create-click="openDialog('create')" />
    <list-table :id="id" :list="list" :list-loading="listLoading" :config="certificatePhotoConfig" height="calc(100% - 157px)"
    ...
    <!-- 上传附件 -->
    <upload-file :dialog-visible="uploadDialogVisible" @close-dialog="uploadDialogVisible = false" @upload-submit="uploadSubmit" />
  </div>
</template>
      ...
      openDialog(name, row) {
        ...
        if (row) {
          getaqglThreePhotoById(row.aqglThreePhotoId).then(response => {
            ...
            this.$refs.editDialog.updataForm(this.editData)
          })
        }
      },
      // 删除
