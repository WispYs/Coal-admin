<template>
  <div class="page-container">
    <filter-bar
      :config="EmergencySuppliesFilterConfig"
    ...
    <list-table
      ...
      :config="EmergencySuppliesTableConfig"
  ...
  methods: {
    ...
    __initOptions() {
      getDictData(9).then(response => {
        ...
        // 更新新增、编辑config数据
        EmergencySuppliesTableConfig.columns.forEach(it => {
      ...
      getDictData(11).then(response => {
        this.formatData(response.data)
        // 更新新增、编辑config数据
        EmergencySuppliesTableConfig.columns.forEach(it => {
          if (it.field === 'deviceType') {
            it.options = response.data
          }
        })
      })
    ...
    initCreateConfig() {
      const createConfig = Object.assign({
        title: '新建',
        width: '1000px',
        form: this.EmergencySuppliesTableConfig.columns
      })
      return createConfig
    },
    // 初始化编辑窗口配置
