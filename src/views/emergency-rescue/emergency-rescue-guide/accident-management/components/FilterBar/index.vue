<template>
  <div class="filter-bar">
    ...
    <div v-if="config.actions && config.actions.length > 0" class="filter-bar__item">
      ...
      <el-button v-if="config.actions.indexOf('refresh') > -1" size="medium" plain @click="refresh()"><i class="el-icon-refresh el-icon--left" />刷新</el-button>
      <el-button type="primary" size="medium" @click="share()"><i class="el-icon-share el-icon--left" />救援过程总览</el-button>
...
export default {
  ...
  methods: {
    ...
    share() {
      this.$router.push({ path: '/emergency-rescue/emergency1/emergency1-1/rescue-process-overview' })
    },
    exportExcel() {
